...
_draftJsUtils.BLOCK_TYPE.CHECKABLE_LIST_ITEM = 'checkable-list-item';
...
var ENTITY_ATTR_MAP = (_ENTITY_ATTR_MAP = {}, _defineProperty(_ENTITY_ATTR_MAP, DOWNLOAD_LINK, { url: 'href', rel: 'rel', title: 'title', className: 'class', size: 'size', name: 'name' }), _defineProperty(_ENTITY_ATTR_MAP, _draftJsUtils.ENTITY_TYPE.LINK, { url: 'href', rel: 'rel', title: 'title', className: 'class' }), _defineProperty(_ENTITY_ATTR_MAP, _draftJsUtils.ENTITY_TYPE.IMAGE, { src: 'src', height: 'height', width: 'width', alt: 'alt', className: 'class', 'data-original-url': 'href' }), _defineProperty(_ENTITY_ATTR_MAP, IFRAME, {
  src: 'src',
  height: 'height',
  width: 'width',
  name: 'name',
...
// Map entity data to element attributes.
function dataToAttr(entityType, entity) {
...
function insertAnchorLink(content) {
  var el = document.createElement('div');
  el.innerHTML = content;
  var child = getChildNodeDeep(el);
  child.innerHTML = child.textContent.replace(URL_REGEX, function (match) {
    return '<a href="' + match + '" target="_blank">' + match + '</a>';
  });
  return el.innerHTML;
}

function getChildNodeDeep(node) {
  ...
  _createClass(MarkupGenerator, [{
    ...
    value: function writeStartTag(blockType) {
      ...
      try {
        for (var _iterator2 = tags[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
          ...
          } else if (blockType === _draftJsUtils.BLOCK_TYPE.CHECKABLE_LIST_ITEM) {
    ...
    value: function renderBlockContent(block) {
      ...
      var ret = entityPieces.map(function (_ref) {
        ...
        var content = stylePieces.map(function (_ref3) {
          ...
          if (entityType != null && entityType === _draftJsUtils.ENTITY_TYPE.LINK) {
            var attrs = ENTITY_ATTR_MAP.hasOwnProperty(entityType) ? dataToAttr(entityType, entity) : null;
        ...
        if (entityType != null && entityType === DOWNLOAD_LINK) {
          ...
          var _attrs = ENTITY_ATTR_MAP.hasOwnProperty(entityType) ? dataToAttr(entityType, entity) : null;
          return '<a href="' + _attrs.href + '" target="_blank" class="uploaded-image"><img src="' + _attrs.src + '" alt="' + _attrs.alt + '" /></a>';
        } else if (entityType != null && entityType === IFRAME) {
          var _attrs2 = ENTITY_ATTR_MAP.hasOwnProperty(entityType) ? dataToAttr(entityType, entity) : null;
          var strAttrs = stringifyAttrs(_attrs2);
      ...
      if (blockType === _draftJsUtils.BLOCK_TYPE.CHECKABLE_LIST_ITEM) {
        var isChecked = this.checkedStateMap[block.getKey()];
